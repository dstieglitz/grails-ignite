...
public class HibernateRegionFactory implements RegionFactory {
    ...
    public void start(SessionFactoryOptions settings, Properties properties) throws CacheException {
        ...
        //
        def gridName = properties.getProperty("hibernate.${HibernateAccessStrategyFactory.IGNITE_INSTANCE_NAME_PROPERTY}")
        def dfltCacheNameProperty = properties.getProperty("hibernate.${HibernateAccessStrategyFactory.DFLT_CACHE_NAME_PROPERTY}")
        def regionCacheProperty = properties.getProperty("hibernate.${HibernateAccessStrategyFactory.REGION_CACHE_PROPERTY}")
        def defaultAccessTypeProperty = properties.getProperty("hibernate.${HibernateAccessStrategyFactory.DFLT_ACCESS_TYPE_PROPERTY}")
        def gridConfigProperty = properties.getProperty("hibernate.${HibernateAccessStrategyFactory.GRID_CONFIG_PROPERTY}")

        log.info "grid name is ${gridName}"
        if (gridName) {
            properties.setProperty(HibernateAccessStrategyFactory.IGNITE_INSTANCE_NAME_PROPERTY, gridName)
        }
        log.info "default cache name is ${dfltCacheNameProperty}"
        if (dfltCacheNameProperty) {
            properties.setProperty(HibernateAccessStrategyFactory.DFLT_CACHE_NAME_PROPERTY, dfltCacheNameProperty)
        }
        log.info "region cache is ${regionCacheProperty}"
        if (regionCacheProperty) {
            properties.setProperty(HibernateAccessStrategyFactory.REGION_CACHE_PROPERTY, regionCacheProperty)
        }
        log.info "defaultAccessTypeProperty is ${defaultAccessTypeProperty}"
        if (defaultAccessTypeProperty) {
            properties.setProperty(HibernateAccessStrategyFactory.DFLT_ACCESS_TYPE_PROPERTY, defaultAccessTypeProperty)
        }
        log.info "grid config is ${gridConfigProperty}"
        if (gridConfigProperty) {
            properties.setProperty(HibernateAccessStrategyFactory.GRID_CONFIG_PROPERTY, gridConfigProperty)
        }
