...
class IgniteCacheConfigurationFactory {
    ...
    static CacheConfiguration getCacheConfiguration(String prefix, String name) {
        ...
        def cacheMode = getConfigValueOrNull("${prefix}.cacheMode")
        if (cacheMode == null) {
            cacheMode = getDefaultCacheConfiguration(name).cacheMode
        }
        log.debug "cacheMode=${cacheMode}"

//        def memoryMode = getConfigValueOrNull("${prefix}.memoryMode", "${prefix}.memoryMode".toString())
//        if (memoryMode == null) {
//            memoryMode = getDefaultCacheConfiguration(name).memoryMode
//        }
//        log.debug "memoryMode=${memoryMode}"

        def atomicityMode = getConfigValueOrNull("${prefix}.atomicityMode")
        if (atomicityMode == null) {
            atomicityMode = getDefaultCacheConfiguration(name).atomicityMode
        }
        log.debug "atomicityMode=${atomicityMode}"

        def writeSynchronizationMode = getConfigValueOrNull("${prefix}.writeSynchronizationMode")
...
//        def offHeapMaxMemory = getConfigValueOrZero("${prefix}.offHeapMaxMemory")
...
//        log.debug "offHeapMaxMemory=${offHeapMaxMemory}"
        ...
        def maxElements = getConfigValueOrNull("${prefix}.maxElements")
        if (maxElements == null) {
            maxElements = getConfigValueOrZero("ignite.defaultCache.maxElements")
        }
        log.debug "maxElements=${maxElements}"

//        def swapEnabled = getConfigValueOrNull("${prefix}.swapEnabled")
//        if (swapEnabled == null) {
//            swapEnabled = getDefaultCacheConfiguration(name).swapEnabled
//        }
//        log.debug "swapEnabled=${swapEnabled}"

        def evictionPolicy = getConfigValueOrNull("${prefix}.evictionPolicy")
//        if (evictionPolicy == null) {
//            evictionPolicy = 'lru'
//        }
        log.debug "evictionPolicy=${evictionPolicy}"

        def statisticsEnabled = getConfigValueOrNull("${prefix}.statisticsEnabled")
        if (statisticsEnabled == null) {
            statisticsEnabled = getDefaultCacheConfiguration(name).statisticsEnabled
        }
        log.debug "statisticsEnabled=${statisticsEnabled}"

        def managementEnabled = getConfigValueOrNull("${prefix}.managementEnabled")
        ...
        def backups = getConfigValueOrZero("${prefix}.backups")
        if (backups == null) {
            backups = getDefaultCacheConfiguration(name).backups
        }
        log.debug "backups=${backups}"

        def copyOnReadEnabled = getConfigValueOrNull("${prefix}.copyOnRead")
...
//        def evictSynchronizedEnabled = getConfigValueOrNull("${prefix}.evictSynchronized")
...
//        config.setEvictSynchronized(evictSynchronized)
    ...
    private static def getConfigValueOrThrowException(name) {
        log.debug "getConfigValueOrThrowException for ${name}"

        return Holders.config.getRequiredProperty(name)
    }

    private static def getConfigValueOrNull(name) {
        log.debug "getConfigValueOrNull for ${name}"

        return Holders.config.getProperty(name, Object, null)
    }

    private static def getConfigValueOrZero(name) {
        log.debug "getConfigValueOrZero for ${name}"

        return Holders.config.getProperty(name, Number, 0)
    }
}
